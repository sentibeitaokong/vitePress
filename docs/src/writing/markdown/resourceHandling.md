...
可以将这些文件放置在`源目录`的 `public` 目录中。例如，如果项目根目录是 `./docs`，并且使用默认源目录位置，那么 public 目录将是 `./docs/public`。
...
```
<!-- withBase 路径会加项目base前缀 -->
在这种情况下，建议使用 VitePress 提供的 `withBase helper` 来包括路径：
